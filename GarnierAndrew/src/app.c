...
t_app*AppCreate(void){
	...
	*pApp=(t_app){
	...
	for(int i=0; i<SHIP_LIFE;i++){
		ContainerPushBack(pApp->pShipsContainer, ShipCreate((SDL_Rect){APP_WINDOW_WIDTH - SHIP_SIZE - SHIP_SIZE*i,
		...
		while(SDL_PollEvent(&event)){
			...
			case SDL_KEYDOWN:
				...
				case SDLK_r:
					...
					break;
				case SDLK_e:
					ContainerFlush(pApp->pAliensContainer);
					break;
				case SDLK_SPACE:{
					/******************************************************
					 * ToDo: taking care of ship rockets firing
					 * */
					if(ContainerCard(pApp->pRocketsContainer) <= 2){
						ContainerPushFront(pApp->pRocketsContainer,RocketCreate((SDL_Rect){ShipHotPoint((t_ship*)ContainerGetBack(pApp->pShipsContainer)).x,
...
Uint32 _AppTimerCallback(Uint32 interval, t_app*pApp){
	...
	if(mIsBitsSet(pApp->m_uStatus,ST_APP_PAUSED)){

		sprintf(pApp->buffer,"GAME PAUSED");
		SDL_SetWindowTitle(pApp->m_pWindow, pApp->buffer);
	...
	sprintf(pApp->buffer," %s Aliens : %d  SCORE : %d  LIFE : %d",APP_WINDOW_TITLE,ContainerCard(pApp->pAliensContainer),pApp->Score,pApp->Life);
	SDL_SetWindowTitle(pApp->m_pWindow, pApp->buffer);
	...
	if(pApp->Life != ContainerCard(pApp->pShipsContainer)){
		ShipEngage((t_ship*)ContainerGetBack(pApp->pShipsContainer),&(SDL_Rect){
																.x = APP_WINDOW_WIDTH/2-SHIP_SIZE,
																.y =APP_WINDOW_HEIGHT -(APP_WINDOW_PADDING_BOT + SHIP_SIZE*2),
																.w = SHIP_SIZE,
																.h = SHIP_SIZE});
		pApp->Life = ContainerCard(pApp->pShipsContainer);
	}
	if(pApp->Life == 1){
		mBitsSet(pApp->m_uStatus,ST_APP_LOOSE);
		return interval;
	}
	...
	ContainerIntersectDelIf(pApp->pRocketsContainer, pApp->pAliensContainer, (t_ptfVV)EntityHit,(t_ptfV)ScoreFunc,pApp);

	ContainerIntersectDelIf(pApp->pShipsContainer, pApp->pAliensContainer, (t_ptfVV)EntityHit,(t_ptfV)ScoreFunc,pApp);
